...
async function fetchOne() {
  ...
  try {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/hadith/${slug}/${num}`)
...
<template>
  <div class="flex flex-col flex-1 bg-background">
    <div class="max-w-3xl mx-auto px-4 py-8 w-full flex-1">
      <Button variant="ghost" size="sm" class="mb-4" @click="router.back()">← Kembali</Button>
      <div v-if="loading" class="py-20 text-center animate-pulse text-muted-foreground">Memuat...</div>
      <div v-else-if="error" class="py-10 text-center text-destructive">{{ error }}</div>
      <template v-else-if="hadith">
        <p class="text-xs text-muted-foreground mb-2">{{ book?.name }} · Hadith #{{ hadith.number }} · {{ hadith.kitabName }} / {{ hadith.babName }}</p>
