...
import {
  useReadingSettings,
  type PaperTheme,
} from '../composables/useReadingSettings';
...
watch(
  () => settings.value.arabicFontSize,
  (v) => (localArabic.value = v)
);
watch(
  () => settings.value.translationFontSize,
  (v) => (localTranslation.value = v)
);
...
<template>
  <Transition name="dialog-fade">
    ...
    >
      ...
      />
      <div
        class="relative w-full max-w-md rounded-2xl bg-card border border-border shadow-xl p-5 max-h-[90vh] overflow-y-auto"
      >
        <div class="flex items-center justify-between mb-4">
          ...
          <Button variant="ghost" size="icon" aria-label="Tutup" @click="close"
            ><X class="w-4 h-4"
          /></Button>
        </div>
        ...
        <div class="space-y-6">
          <div>
            <label class="text-sm font-medium text-foreground"
              >Ukuran Arab: {{ settings.arabicFontSize }}px</label
            >
            <input
              type="range"
              :min="20"
              :max="40"
              :step="1"
              v-model.number="localArabic"
              class="w-full mt-2 accent-primary"
            />
            <div
              class="flex justify-between text-xs text-muted-foreground mt-1"
            >
              <span>20px</span><span>40px</span>
            </div>
          </div>

          <div>
            <label class="text-sm font-medium text-foreground"
              >Ukuran Terjemahan: {{ settings.translationFontSize }}px</label
            >
            <input
              type="range"
              :min="11"
              :max="16"
              :step="1"
              v-model.number="localTranslation"
              class="w-full mt-2 accent-primary"
            />
            <div
              class="flex justify-between text-xs text-muted-foreground mt-1"
          ...
          <div class="flex items-center justify-between">
            ...
            >
              <span
                ...
                :class="settings.showArabic ? 'translate-x-6' : 'translate-x-1'"
              />
            </button>
          </div>
          ...
          <Button variant="outline" class="w-full gap-2" @click="resetSettings"
            ><RotateCcw class="w-4 h-4" /> Reset ke Default</Button
          >
        </div>
      </div>
    </div>
