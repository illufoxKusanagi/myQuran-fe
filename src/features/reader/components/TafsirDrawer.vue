...
function getFocusable(container: HTMLElement): HTMLElement[] {
  return Array.from(container.querySelectorAll<HTMLElement>('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')).filter(
      ...
      >
        <div class="sticky top-0 bg-card flex items-start justify-between p-4 border-b border-border z-10">
          <div>
            ...
            <p class="text-xs text-muted-foreground">Ayah {{ ayah?.ayahNumber }}</p>
          </div>
          <Button ref="closeBtnRef" variant="ghost" size="sm" @click="emit('close')">Close</Button>
        </div>
        <div v-if="ayah" class="p-5 space-y-5">
          <div class="rounded-lg bg-muted p-4">
            ...
            <p class="text-muted-foreground text-sm mt-2 italic">{{ ayah.translation }}</p>
          </div>
          <div>
            <p class="text-sm font-semibold text-foreground mb-2">Tafsir Wajiz</p>
            <p class="text-sm text-muted-foreground leading-relaxed">{{ ayah.wajizTafsir || 'Not available.' }}</p>
          </div>
          <div class="border-t border-border pt-4">
            <p class="text-sm font-semibold text-foreground mb-2">Tafsir Tahlili</p>
            <p class="text-sm text-muted-foreground leading-relaxed">{{ ayah.tahliliTafsir || 'Not available.' }}</p>
          </div>
        </div>
      </div>
    </div>
