...
const props = defineProps<{
  ...
  jumpToAyah: (ayahNumber: number) => boolean | Promise<boolean>;
}>();
...
function handleSubmit() {
  ...
  const hadithMatch = q.match(
    /^(bukhari|muslim|nasai|abudaud|tirmidzi|ibnumajah|malik)[:\s]+(\d+)$/i
  );
  if (hadithMatch) {
    const b = hadithMatch[1].toLowerCase();
    const num = Number(hadithMatch[2]);
    close();
    router.push({
      name: 'hadith-detail',
      params: { book: b, number: String(num) },
    });
    return;
  }
  ...
  error.value = 'Format: 2:255, 18:10, juz 5, bukhari 1, atau yasin/kursi';
}
</script>
...
<template>
  <Transition name="dialog-fade">
    ...
    >
      ...
      <div
        class="relative w-full max-w-lg rounded-2xl bg-card border border-border shadow-xl p-5"
      >
        <div class="flex items-center gap-3 mb-3">
          <Search class="w-5 h-5 text-muted-foreground" />
          <h2 class="font-semibold text-foreground">Lompat Cepat</h2>
          <span
            class="ml-auto text-xs text-muted-foreground border border-border rounded px-1.5 py-0.5"
            >Ctrl+K</span
          >
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <p class="text-xs text-muted-foreground mt-2">
            Tekan Enter. Juz 1–30, Surah 1–114, atau kata kunci populer.
          </p>
